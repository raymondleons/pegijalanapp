...
// --- Komponen Tampilan Saat Belum Login ---
const LoggedOutView = ({ navigation, t }) => (
  <>
    <View style={styles.card}>
      <View style={styles.promptContent}>
        <View style={styles.avatar}>
          <Image source={require('../assets/icons/user.png')} style={styles.avatarIcon} />
        </View>
...
// --- Komponen Tampilan Saat Sudah Login dengan Skeleton ---
const LoggedInView = ({ navigation, t }) => {
  ...
  if (isLoadingProfile) {
    // Tampilkan skeleton loading profil dan menu
    return (
      <View style={{ flex: 1, justifyContent: 'flex-start', paddingTop: SIZES.padding * 2 }}>
        <ProfileSkeleton />
        <View style={styles.menuContainer}>
          {[1, 2, 3, 4, 5, 6].map((key) => (
            <React.Fragment key={key}>
              <MenuItemSkeleton />
              {key < 6 && <View style={styles.divider} />}
            </React.Fragment>
          ))}
        </View>
      </View>
      ...
      <View style={styles.referContainer}>
        <TouchableOpacity style={styles.referBanner}>
          <View style={styles.referTextContainer}>
            <Text style={styles.referTitle}>{t('refer_and_earn')}</Text>
            <Text style={styles.referAmount}>&gt; Rp. 26.000.000</Text>
          </View>
        </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: SIZES.padding2 }}>
        {userToken ? <LoggedInView navigation={navigation} t={t} /> : <LoggedOutView navigation={navigation} t={t} />}
      </ScrollView>
    </SafeAreaView>
  );
};
